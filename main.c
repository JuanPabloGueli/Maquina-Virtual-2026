...
void leeVmx_MV1(FILE *arch, maquinaV *mv) {
    ...
    if(tamCod > MEM_SIZE){
        ...
        for (i=2 ; i < CANT_SEG; i++)
            for (j=0; j < 2; j++)
                mv->tablaSeg[i][j] = -1;

        /*for (i=0 ; i < CANT_SEG; i++){                                           /// print TABLA DE SEGMENTOS
            for (j=0; j < 2; j++)
                printf("%04X   ",mv->tablaSeg[i][j]);
            printf("\n");
            }*/
...
void tabla_segmentos_2 (maquinaV *mv, int VectorSegmentos[], unsigned int TopeVecSegmentos){
                ...
                if (archvmx != NULL){
                    ...
                    if (Version == 1){
                        ...
                        if (Version == 2){
                            ...
                            tabla_segmentos_2 (mv,VectorSegmentos,TopeVecSegmentos);
...
int main(int argc, char *argv[]) {
    ...
    memset(mv.mem, 0 ,MEM_SIZE); // setea toda la memoria con ceros
